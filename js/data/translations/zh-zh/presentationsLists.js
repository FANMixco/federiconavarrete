const presentationsVideos = {
	"presentations": [
		{
			"youTubeID": "IcWZ962uYy0",
			"title": "<a class='text-material-link-dark' href='https://www.citython.eu/2021eindhoven' rel='noreferrer' target='_blank'>获胜团队|格林霍芬团队|挑战3-让城市健康-2021埃因霍温城市马拉松（英文）</a>"
		},
		{
			"youTubeID": 'XJyWAErkNtY',
			"title": "与Natalia Wiechowski博士的真理时刻（英文）"
		},
		{
			"youTubeID": '-c2zayFv14Y',
			"title": "缺失变量（英文）"
		}
	],
	"isVisible": true
};

const youtubeTrainings = {
	"presentations": [
		{
			"youTubeID": 'VJF9NZOehTE',
			"title": "当低代码遇到#ChatGPT！你能用低代码创建你的第一个聊天机器人吗？（英文）"
		},
		{
			"youTubeID": "oPg8aIlZJds",
			"title": "让我们用Prezi视频制作精彩的演示文稿（英文）"
		},
		{
			"youTubeID": '9mXanZhm9eo',
			"title": "是时候升级了⏫ 您的LinkedIn个人资料！（英文）"
		}
	],
	"isVisible": true
};

const presentationsLinks = {
	"presentations": [
		{
			"link": 'https://bit.ly/3zURwzI',
			"title": "终极电梯演讲（英文）",
			"imgID": "imgPPT2",
			"imgBasicName": "elevator",
			"imgFormat": "jpg"
		},
		{
			"link": 'https://bit.ly/2WGzfb1',
			"title": "机器人是你的朋友吗？（英文）",
			"imgID": "imgPPT1",
			"imgBasicName": "rfriends",
			"imgFormat": "jpg"
		}
	],
	"isVisible": true
};